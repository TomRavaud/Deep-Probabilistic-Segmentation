...
def extract_contour_points_and_normals(mask, num_points_on_contour=200):
    ...
    # ax.plot(*points[:].T)
    # points_test = np.asarray(interpolate.splev(u[tii], tck, der=0)).T
    # ax.plot((points[:, 0], points_test[:, 0]), (points[:, 1], points_test[:, 1]))
    # ax.quiver(points[:, 0], points[:, 1], normals[:, 0], normals[:, 1])
    # ax.axis('equal')
    # plt.show()
...
def extract_contour_lines(points, normals, line_size_half):
    ...
    try:
        ...
        exit(1)
    # print(np.array(outer_lines).shape)
    # print(np.array(inner_lines).shape)
